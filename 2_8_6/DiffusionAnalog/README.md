...
INPUT_FILE_NAME = "DAInput.xlsx"
SHEET1_NAME = "計算条件"
SHEET2_NAME = "上流端流量の経時変化"
OUTPUT_FILE_NAME = "DA/ResDA.xlsx"
EPS_DEPTH = 1e-15
...
class SetData:
    ...
    def __init__(self):
        ...
        # dh/dxの配列の作成
        self.dhdx = np.zeros(self.totalDivNo+1)
        # 水深h,流量Qの配列を作成
...
$\frac{\partial A}{\partial t}+\frac{\partial Q}{\partial t} = 0$・・・式(35)

$Q = \frac{1}{n} A R^{2/3}\left(I_b-\frac{\partial h}{\partial x}\right)^{1/2}$・・・式(41)
...
$I_b-\frac{(A_1^{t+\Delta t}/B)-(A_0^{t+\Delta t}/B)}{\Delta x}-\frac{n^2}{{R_0^{t+\Delta t}}^{4/3}}\left(\frac{Q_0^{t+\Delta t}}{A_0^{t+\Delta t}}\right)^2 =0 $・・・式(42)

式(42)より，$A_0^{t+\Delta t}$を算出．
...
class DiffusionAnalog(SetData):
    ...
    def __updateValue(self):
        ...
        self.dhdx[1:-1] = (self.h[2:]-self.h[:-2])/(2.*self.dx)
        self.dhdx[-1] = (self.h[-1]-self.h[-2])/self.dx
        self.Q[1:] = FlowParam.calcQ0(self.n, self.A[1:], self.R[1:],
                                      self.Ib-self.dhdx[1:])
